...
export async function POST(request: NextRequest) {
  ...
  const stage = (SHOP_STAGES as readonly string[]).includes(body.stage)
    ? body.stage
    : "design_approved";
  const fab_step = (SHOP_FAB_STEPS as readonly string[]).includes(body.fab_step)
    ? body.fab_step
    : null;
  const { data, error } = await auth.supabase
    .from("shop_queue")
    .insert({
      ...
      fab_step,
      craftsman_name:
        typeof body.craftsman_name === "string"
          ? body.craftsman_name.trim() || null
          : null,
