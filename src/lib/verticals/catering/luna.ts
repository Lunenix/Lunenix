...
export async function executeCateringLunaTool(
  supabase: Db,
  workspaceId: string,
  name: string,
  args: Record<string, unknown>
): Promise<VerticalExecutionResult | null> {
  if (name === "list_catering_events") {
    const { data, error } = await supabase
      .from("catering_events")
      .select("title, event_on, venue_name, guest_count, service_style, status")
      .eq("workspace_id", workspaceId)
    ...
    const lines = (data ?? []).map(
      (e: {
        ...
        `${e.title} ${e.event_on ?? "no date"} at ${e.venue_name ?? "no venue"}, ${e.guest_count ?? "?"} guests, ${e.service_style}, ${e.status}`
    );
    return {
      ok: true,
      summary: lines.length
        ? lines.join(". ")
        : "No catering events in this workspace.",
    };
  }

  if (name !== "catering_log_event") return null;

  const flat = flattenCateringSpecs(args);
  ...
  const dates = cateringEventDateFields(flat);
  const styleRaw = str(flat, "service_style") ?? "buffet";
  const service_style = (CATERING_STYLES as readonly string[]).includes(styleRaw)
    ? styleRaw
    : "buffet";
  const event_type = (CATERING_EVENT_TYPES as readonly string[]).includes(
    str(flat, "event_type") ?? ""
  )
    ? str(flat, "event_type")
    : "wedding";
  const title =
    str(flat, "title") ??
    `${contact && "label" in contact ? contact.label : "Client"} catering`;
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      event_type,
      service_style,
      dietary_notes: str(flat, "dietary_notes"),
  ...
  return {
    ok: true,
    summary: `Logged ${CATERING_STYLE_LABELS[service_style as keyof typeof CATERING_STYLE_LABELS]} for ${data.title}, ${guestCount} guests.`,
  };
}
